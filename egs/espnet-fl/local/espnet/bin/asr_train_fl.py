...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument('--fbank-fmax', type=float, default=None,
                        help='')
    parser.add_argument('--pretrain-model', default='', help='Load pretrained model')
    parser.add_argument('--iterval', default=None, type=int, help='used for stopping every iterval iteration')
    parser.add_argument('--itermod',default=0, type=int, help='selecting if using iter_stopping')
    parser.add_argument('--epstop',default=None, type=int, help='in itermod=1 check if epoch stop or iteration stop')
...
def main(cmd_args):
    ...
    if args.num_spkrs == 1:
        if args.backend == "chainer":
            ...
            from espnet.asr.pytorch_backend.asrfl import train
            train(args)
        else:
