...
from espnet.asr.asr_utils_fl import adadelta_eps_decay
from espnet.asr.asr_utils_fl import add_results_to_json
from espnet.asr.asr_utils_fl import CompareValueTrigger
from espnet.asr.asr_utils_fl import format_mulenc_args
from espnet.asr.asr_utils_fl import get_model_conf
from espnet.asr.asr_utils_fl import plot_spectrogram
from espnet.asr.asr_utils_fl import restore_snapshot
from espnet.asr.asr_utils_fl import snapshot_object
from espnet.asr.asr_utils_fl import torch_itersnapshot
from espnet.asr.asr_utils_fl import torch_load
from espnet.asr.asr_utils_fl import torch_load_pretrain_audio_audio
from espnet.asr.asr_utils_fl import torch_resume
from espnet.asr.asr_utils_fl import torch_snapshot
...
from espnet.utils.training.train_utils_fl import check_early_stop
from espnet.utils.training.train_utils_fl import set_early_stop
from espnet.utils.training.train_utils_fl import set_iter_stop
from espnet.utils.training.train_utils_fl import set_epoch_stop

from espnet.utils.training.train_utils_fl import set_next_epoch_stop
import matplotlib
matplotlib.use('Agg')
...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        if self.grad_noise:
            from espnet.asr.asr_utils_fl import add_gradient_noise
...
def train(args):
    ...
    if args.opt == 'adadelta':
        ...
        from espnet.nets.pytorch_backend.transformer.optimizer_sgd import get_std_opt
        optimizer = get_std_opt(model, args.adim, args.transformer_warmup_steps, args.transformer_lr)
    else:
    ...
    if args.startup is not None:
        with open(os.path.join(args.outdir,'itercount.txt'), 'w') as iterfile:
            print(len(train),file=iterfile)
        iterfile.close()
    ...
    if args.pretrain_model:
        logging.info('load pretrained model from %s' %args.pretrain_model)
        torch_load_pretrain_audio_audio(args.pretrain_model,trainer)
    ...
    #trainer.extend(snapshot_object_iteration(model, 'snapshot.iter',args.iterval), trigger=(args.iterval, 'iteration'))
    ...
    if args.itermod == 0:
        ...
        if args.epstop == 1:
            #print("EPTRIGGER",args.epstop)
            trainer.extend(torch_snapshot(), name="epochtrigger", trigger=(1, 'epoch'))
            set_epoch_stop(trainer,args)
        else:
            #print("ITERTRIGGER",args.epstop)
            trainer.extend(torch_itersnapshot(),name="itertrigger", trigger=(1, 'iteration'))
            set_iter_stop(trainer,args)
    elif args.itermod == 2:
        trainer.extend(torch_snapshot(), trigger=(1, 'epoch'))
        set_next_epoch_stop(trainer,args)
