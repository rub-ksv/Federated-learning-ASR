...
def torch_itersnapshot(savefun=torch.save,
    ...
    """
    @extension.make_extension(trigger=(1, 'iteration'), priority=-100)
    def torch_snapshot(trainer):
        _torch_iter_snapshot_object(trainer, trainer, filename.format(trainer), savefun)

    return torch_snapshot

def _torch_iter_snapshot_object(trainer, target, filename, savefun):
...
def get_avmodel_conf(model_path, conf_path=None):
    """Get model config information by reading a model config file (model.json).
...
def torch_save_av(path, model1, model2):
    """Save torch model states.
    ...
    """
    if hasattr(model, 'module'):
        torch.save({'modelA_state_dict': model1.module.state_dict(),'modelV_state_dict': model2.module.state_dict()}, path)
    else:
        torch.save({'modelA_state_dict': model1.state_dict(),'modelV_state_dict': model2.state_dict()}, path)
...
def snapshot_avobject(target1, target2, filename):
    """Returns a trainer extension to take snapshots of a given object.
    ...
    """
    @extension.make_extension(trigger=(1, 'epoch'), priority=-100)
    def snapshot_avobject(trainer):
        torch_save_av(os.path.join(trainer.out, filename.format(trainer)), target1, target2)

    return snapshot_object


def snapshot_object_iteration(target, filename, val):
    """Returns a trainer extension to take snapshots of a given object.
    ...
    """
    @extension.make_extension(trigger=(1, 'iteration'), priority=-100)
    def snapshot_object_iteration(trainer):
        _torch_snapshot_object(os.path.join(trainer.out, filename.format(trainer)), target)

    return _snapshot_object_iteration
...
def torch_resume(snapshot_path, trainer):
    ...
    print(snapshot_dict['optimizer'])

    # delete opened snapshot
    del snapshot_dict

def torch_load_pretrain_audio(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
    ...
    for name, param in snapshot_dict.items():
        if name not in trainer.updater.model.aencoder.state_dict():
            continue
        else:
            trainer.updater.model.aencoder.state_dict()[name].copy_(param)
    print('load for audio model')
    for name, param in snapshot_dict.items():
        #param.requires_grad = False
        if name not in trainer.updater.model.vencoder.state_dict():
            continue
        else:
            try:
                trainer.updater.model.vencoder.state_dict()[name].copy_(param)
            except:
                pass
...
def torch_load_pretrain_audio_audio(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
    ...
    #trainer.updater.model.load_state_dict(snapshot_dict)
    for name, param in snapshot_dict.items():
        if name not in trainer.updater.model.state_dict():
            continue
            print('load layer wrong')
        else:
            trainer.updater.model.state_dict()[name].copy_(param)
    print('load for audio model')


    del snapshot_dict

def torch_load_pretrain_audio_video(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
