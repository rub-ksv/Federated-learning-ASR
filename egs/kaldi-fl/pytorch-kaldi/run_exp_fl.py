...
# October 2018
##########################################################
# Edited to run as a FL simulation
# compare with difftool to run_exp.py to see the changes
#
...
for ep in range(N_ep):
    ...
    for tr_data in tr_data_lst:
        ...
        for ck in range(N_ck_tr):
            ...
            # Exit if chunk number reached the specified value
            chunk_count = int(config['exp']['fl_ck_count'])
            total_ck = N_ck_tr * ep + ck + 1
            #print(f"total_ck {total_ck}, ep {ep}, ck {ck}")
            #print(f"chunk_count {chunk_count}")
            
            try:
                with open(f"{out_folder}/exp_files/train_step_counter.txt", 'r') as infile:
                    c_ck = int(infile.read())
            except:
                c_ck=0 
            #print(f"c_ck {c_ck}")
            
            #print(f"{total_ck} >= {c_ck + chunk_count}")
            
            if total_ck >= c_ck + chunk_count:
                with open(f"{out_folder}/exp_files/train_step_counter.txt", 'w') as outfile:
                    outfile.write(f"{total_ck}")
                print(f"exit after chunk {ck} in epoch {ep}.")
                exit()
